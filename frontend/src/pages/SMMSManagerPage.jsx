...
import { Card, Table, Tag, Button, Tabs, Space, Progress, Drawer, Descriptions, Divider, Alert, Row, Col, message, Input, Select } from 'antd';
import { ThunderboltOutlined, PlusOutlined, SendOutlined, EyeOutlined, RobotOutlined, WarningOutlined, SafetyCertificateOutlined, ToolOutlined, BlockOutlined, DownloadOutlined, SearchOutlined, CalendarOutlined } from '@ant-design/icons';
...
export const SMMSManagerPage = () => {
  ...
  const loadDefects = () => {
    defectService.getSMMSDefects()
      .then(list => {
        setDbDefects(Array.isArray(list) ? list : []);
      })
      .catch(err => {
        console.error('Failed to load SMMS defects from smms_defects collection:', err);
        setDbDefects([]);
      });
  };
  ...
  // Use only live defects from DB collection (smms_defects)
  ...
  const handleExportExcel = () => {
    const headers = [
      ...
      'Due Date', 'Overdue Days', 'Power Supply Type', 'Interlocking Type', 'Comm Link Status', 'Priority Score', 'Priority Class', 'Predicted Resolution Hrs', 'Risk If Delayed', 'Confidence Score'
    ];
    const rows = finalFilteredData.map(d => [
      d.Defect_ID || d.id || d.defectId, d.Section_ID || d.sectionId || 'SEC-NDLS-CNB-DN', d.Station1 || d.station1 || 'NDLS', d.Station2 || d.station2 || 'CNB', d.Chainage_KM || d.chainageKm || '142.100', d.Asset_ID || d.assetId || 'SIG-PT-102B', d.Asset_Type || d.assetType || 'Point Machine', d.Defect_Type || d.defectType || 'Point Machine Failure', d.Severity_Level || d.severityLevel || 'Critical', d.Detection_Method || d.detectionMethod || 'SSI Log', d.Reported_Date || d.reportedDate || '2026-09-01',
      d.Due_Date || '2026-09-02', d.Overdue_Days || d.overdueDays || 0, d.Power_Supply_Type || '110V AC Signalling Power', d.Interlocking_Type || 'Electronic Interlocking (EI)', d.Communication_Link_Status || 'Degraded', d.Priority_Score || d.priorityScore || 85.0, d.Priority_Class || d.priorityClass || 'Critical', d.Predicted_Resolution_Time_Hours || d.predictedResolutionTimeHours || 3.0, d.Risk_If_Delayed || d.riskIfDelayed || 'High Risk of Point Indication Failure', d.Confidence_Score || '95%'
    ]);
    exportReportToExcel('SMMS_Signal_Defects_Full_Schema', 'SMMS_Defects', headers, rows);
  };
  ...
  const handleSendToBDMS = async (record, dateRangePayload) => {
    ...
    try {
      ...
      message.loading({ content: `Evaluating XGBoost Joint Placement for SMMS Defect ${defectId}...`, key: `send-${defectId}` });
      const res = await blockService.sendDefectToBlockPlanning('SMMS', defectId, dateRangePayload);
      ...
      console.error('Error sending SMMS defect to BDMS:', err);
      message.error({
  ...
  const columns = [
    {
      ...
      render: (t, record) => (
        <strong style={{ color: '#d97706' }}>
          {t || record.defectType || 'Point Machine Failure'}
        </strong>
      )
    },
    {
      title: 'Location / Mast',
      key: 'location',
      render: (_, r) => r.location || `KM ${r.chainageKm || r.Chainage_KM || '142.100'} (${r.Asset_ID || r.assetId || 'SIG-PT-102B'})`
    },
    {
    ...
    },
    {
      ...
      key: 'Priority_Score',
      render: (score, record) => {
        const pScore = Number(score || record.priorityScore || 85.0);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        ...
        <p style={{ color: 'var(--ir-text-sub)', margin: '4px 0 16px 0', fontSize: 13 }}>
          Official CRIS SMMS Protocol: Point machines, track circuits, signals, EI, &amp; XGBoost joint disconnection block optimizer
        </p>
        ...
        {!isCOA && (
          <Space size="middle">
            ...
            >
              Log Signal Defect (Manual Input)
            </Button>
      ...
      <Card styles={{ body: { padding: 16 } }} style={{ borderRadius: 10, border: '1px solid var(--ir-border)' }}>
        ...
        <Row gutter={[16, 16]} align="middle">
          <Col xs={24} sm={16} md={10} lg={8}>
            <Input
              placeholder="Search SMMS defects..."
      ...
      {/* Comprehensive SMMS Defect Inspector & AI Analytics Drawer */}
      <Drawer
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#d97706' }}>
            <ThunderboltOutlined />
            <span>SMMS Defect Telemetry &amp; Decision-Support: {selectedDetailRecord?.defectId || selectedDetailRecord?.Defect_ID || selectedDetailRecord?.id}</span>
          </div>
        }
        placement="right"
        width={620}
        ...
        {selectedDetailRecord && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <Alert
              type="info"
              showIcon
              message="CRIS SMMS SIGNAL &amp; TELECOM DATABASE RECORD &amp; AI RESULTS"
              description="Record loaded directly from MongoDB smms_defects collection with AI-evaluated priority scores and resolution metrics."
              style={{ fontSize: 12 }}
            />
            ...
            <div>
              ...
              <Descriptions column={2} bordered size="small">
                ...
                <Descriptions.Item label="Defect Type">
                  <strong style={{ color: '#d97706' }}>{selectedDetailRecord.defectType || selectedDetailRecord.Defect_Type || 'N/A'}</strong>
                </Descriptions.Item>
                <Descriptions.Item label="Detection Method">
                  {selectedDetailRecord.detectionMethod || selectedDetailRecord.Detection_Method || 'Remote Diagnostic System'}
                </Descriptions.Item>
                ...
                <Descriptions.Item label="Power Supply Type">
                  <strong>{selectedDetailRecord.powerSupplyType || selectedDetailRecord.Power_Supply_Type || '110V AC Signalling Power'}</strong>
                </Descriptions.Item>
                <Descriptions.Item label="Interlocking Type">
                  <strong>{selectedDetailRecord.interlockingType || selectedDetailRecord.Interlocking_Type || 'Electronic Interlocking (EI)'}</strong>
                </Descriptions.Item>
                <Descriptions.Item label="Comm Link Status">
                  <Tag color={(selectedDetailRecord.communicationLinkStatus || selectedDetailRecord.commLinkStatus) === 'Healthy' ? 'success' : (selectedDetailRecord.communicationLinkStatus || selectedDetailRecord.commLinkStatus) === 'Degraded' ? 'warning' : 'error'}>
                    ● {selectedDetailRecord.communicationLinkStatus || selectedDetailRecord.commLinkStatus || selectedDetailRecord.Communication_Link_Status || 'Normal'}
                  </Tag>
                </Descriptions.Item>
              ...
              <Row gutter={[12, 12]}>
                  ...
                  <div style={{
                    ...
                    {selectedDetailRecord.severityReason || selectedDetailRecord.Severity_Reason || (
                      selectedDetailRecord.defectType
                        ? `Classified based on SMMS safety standards and operational risk for ${selectedDetailRecord.defectType}.`
                        : 'Not available'
                    )}
                  </div>
                </Col>
                ...
                <Col span={12}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: '#059669' }}>
                    {selectedDetailRecord.workDurationHrs || selectedDetailRecord.predictedResolutionTimeHours || 3.0} Hours
                  </div>
                </Col>
                <Col span={24}>
                  <div style={{ fontSize: 11, color: '#64748b', marginBottom: 2 }}>Risk If Delayed</div>
                  <div style={{ fontSize: 12, color: '#b91c1c', fontWeight: 600 }}>
                    {selectedDetailRecord.riskIfDelayed || selectedDetailRecord.Risk_If_Delayed || 'Signalling point/interlocking failure risk.'}
                  </div>
                </Col>
              </Row>
            </Card>
      ...
      <AddDefectModal
        ...
        onDefectAdded={(newDefect) => setDbDefects(prev => [newDefect, ...prev])}
      />
      <DispatchDefectModal
